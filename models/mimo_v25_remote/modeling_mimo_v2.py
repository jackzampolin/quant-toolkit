...
class MiMoVisionPatchEmbed(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        weight = self.proj_weight_linear_format
        if weight is None:
            weight = self.proj.weight.view(self.embed_dim, -1)
        return F.linear(hidden_states.to(dtype=target_dtype), weight)
...
def _mimo_vision_reference_attention_segment_impl(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    sinks: torch.Tensor,
    ...
    q_len = q.shape[0]
    k_len = k.shape[0]
    scores = torch.einsum("qhd,khd->hqk", q, k) * scaling
    ...
    if use_sinks:
        num_heads = q.shape[1]
        sink_logits = sinks.to(device=q.device, dtype=torch.float32).view(num_heads, 1, 1)
        scores = torch.cat([scores, sink_logits.expand(num_heads, q_len, 1)], dim=-1)
...
class MiMoVisionAttention(nn.Module):
    ...
    def _window_sides(self) -> tuple[int, int]:
        if isinstance(self.window_size, (tuple, list)):
            if len(self.window_size) != 2:
                raise ValueError(f"window_size must have 2 elements, got {self.window_size}")
            return int(self.window_size[0]), int(self.window_size[1])
        window_size = int(self.window_size)
        return window_size, window_size

    def _attention_pytorch(
        self,
        q: torch.Tensor,
        k: torch.Tensor,
        v: torch.Tensor,
        *,
        ...
        left, right = -1, -1
        use_sinks = False
        sinks = q.new_empty((0,), dtype=torch.float32)
        if not full_attn:
            left, right = self._window_sides()
            if self.sinks is not None:
                use_sinks = True
                sinks = self.sinks

        return _mimo_vision_reference_attention_segment_impl(
            q,
            k,
            v,
            sinks,
    ...
    def forward(
        ...
        if hidden_states.dim() != 3:
            raise ValueError(f"Vision attention expects [B, S, H] or [S, H], got {tuple(hidden_states.shape)}")
        ...
        q = qkv[..., :q_dim].reshape(bsz * seq_len, self.num_heads, self.head_dim)
        k = qkv[..., q_dim : q_dim + kv_dim].reshape(bsz * seq_len, self.num_kv_heads, self.head_dim)
        v = qkv[..., q_dim + kv_dim :].reshape(bsz * seq_len, self.num_kv_heads, self.head_dim)
        ...
        for q_c, k_c, v_c in zip(q_chunks, k_chunks, v_chunks):
            attn_out = self._attention_pytorch(q_c, k_c, v_c, full_attn=full_attn)
            outputs.append(attn_out)

        attn_output = torch.cat(outputs, dim=0)
        attn_output = attn_output.reshape(bsz, seq_len, -1)
        attn_output = self.proj(attn_output)
        return attn_output.squeeze(0) if squeeze_batch else attn_output
...
class MiMoVisionBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.norm1 = MiMoV2RMSNorm(dim, eps=rms_norm_eps)
        self.norm2 = MiMoV2RMSNorm(dim, eps=rms_norm_eps)
    ...
    def forward(
        ...
        if hidden_states.dim() == 2:
            ...
            squeeze_batch = True
        if hidden_states.dim() != 3:
            raise ValueError(f"Vision block expects [S, B, H] or [S, H], got {tuple(hidden_states.shape)}")

        seq_len, batch_size, hidden_size = hidden_states.shape
        hidden_states_2d = hidden_states.reshape(-1, hidden_size)
        normed = self.norm1(hidden_states_2d).reshape(seq_len, batch_size, hidden_size)
        normed = normed.transpose(0, 1)

        attn = self.attn(
            normed, cu_seqlens=cu_seqlens,
            position_embeddings=position_embeddings, full_attn=full_attn,
        )
        attn = attn.transpose(0, 1).reshape(-1, hidden_size)

        hidden_states_2d = hidden_states_2d + attn
        mlp_input = self.norm2(hidden_states_2d).reshape(seq_len, batch_size, hidden_size)
        hidden_states = hidden_states_2d.reshape(seq_len, batch_size, hidden_size) + self.mlp(mlp_input)
        return hidden_states.squeeze(1) if squeeze_batch else hidden_states
...
class MiMoVisionPatchMerger(nn.Module):
    def __init__(self, dim: int, context_dim: int, spatial_merge_size: int = 2):
        ...
        self.ln_q = MiMoV2RMSNorm(context_dim, eps=1e-6)
...
class MiMoVisionTransformer(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        patch_device = self.patch_embed.proj.weight.device
        if patch_device.type == "meta":
            x = pixel_values.to(dtype=self.dtype)
        else:
            x = pixel_values.to(device=patch_device, dtype=self.dtype)
...
def _mimo_audio_reference_attention_segment_impl(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    scaling: float,
    ...
    if causal or left != -1 or right != -1:
        q_pos = torch.arange(q_len, device=q.device).unsqueeze(1)
        k_pos = torch.arange(k_len, device=q.device).unsqueeze(0)
        q_aligned = q_pos + k_len - q_len
        keep = torch.ones((q_len, k_len), dtype=torch.bool, device=q.device)
        if causal:
            keep &= k_pos <= q_aligned
        if left != -1:
            keep &= k_pos >= q_aligned - left
        if right != -1:
            keep &= k_pos <= q_aligned + right
...
class AudioTokenizerAttention(nn.Module):
    ...
    def forward(self, hidden_states, cu_seqlens, max_seqlen, rope_position_embeddings=None):
        ...
        for i in range(num_seqs):
            ...
            out = _mimo_audio_reference_attention_segment_impl(
                q[start:end],
                k[start:end],
                v[start:end],
                self.scaling,
